...
n_missing_age <- length(which(is.na(HPS$Age))) # 0
n_missing_calldate <- length(which(is.na(HPS$Call_Date))) # 0
n_missing_calltime <- length(which(is.na(HPS$Call_Time))) # 0
n_missing_PickUpLocation <- length(which(is.na(HPS$PickupLocationDescription))) # 0
