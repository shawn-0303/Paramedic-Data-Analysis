...
CKL <- CKL |> mutate(across(c(TransportID, PrimaryProblemCode, SecondaryProblemCode, OtherSecondaryProblemCode), as.character))

# Load HPS data
load(here("data", "HPS.rda"))
HPS <- HPS |> mutate(across(c(TransportID, PrimaryProblemCode, SecondaryProblemCode, OtherSecondaryProblemCode), as.character))

# Load Chris PCCP data
